...
public class RuleSet {
    ...
    private final Predicate<Version> rule;

    private RuleSet(Predicate<Version> rule) {
        this.rule = rule;
    }
    ...
    public static class Builder implements Parser<RuleSet> {
        ...
        public RuleSet parse(String input) {
            ...
            while (!stream.positiveLookahead(EOI)) {
                ...
                else if(stream.lookahead().type == EOI) throw new UnexpectedTokenException(stream.lookahead(), LEFT_PAREN, LEFT_SQBR);
            }
